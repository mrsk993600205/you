...
// disabled: cloud — isCloudUserPlaylist always false
const isCloudUserPlaylist = computed(() => false)
/*
...
const fetchPlaylistSongs = async () => {
  try {
    ...
    // disabled: cloud — isCloudUserPlaylist always false now
...
const fetchCloudUserPlaylist = async (_reset = false) => {
  return
}
...
const fetchPlaylistShareSongs = async (_reset = false) => {
  return
}
...
const handleAddBatchToSongList = async (batchSongs: MusicItem[], playlist: SongList) => {
  ...
  try {
    ...
    // disabled: cloud/sharing - cloudSongs and cloud sync removed

    // disabled: cloud/sharing - isCloudOnly branch removed
    ...
    if (result.success) {
      ...
      // disabled: cloud/sharing - cloud sync removed
    } else {
...
const handleRemoveFromLocalPlaylist = async (song: MusicItem) => {
  // disabled: cloud/sharing - isCloudUserPlaylist branch removed
...
const handleRemoveBatchSelected = async (batchSongs: any[]) => {
  ...
  // disabled: cloud/sharing - removeCloudSongs and isCloudUserPlaylist branch removed

  try {
    const mids = batchSongs.map((s: any) => s.songmid)
    // disabled: cloud/sharing - cloud_user route branch removed
    {
...
const handleFileSelect = async (event: Event) => {
  ...
  // disabled: cloud/sharing - isCloudUserPlaylist cover upload removed
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          // disabled: cloud/sharing - cloud cover sync removed
        } else {
...
const playAll = (shouldShuffle = false) => {
  return async () => {
    ...
    if (!isLocalPlaylist.value && hasMore.value) {
      ...
      while (hasMore.value) {
        // disabled: cloud — cloud_user type no longer handled
        // if (route.query.type === 'cloud_user') {
        //   await fetchCloudUserPlaylist(false)
        // } else
        if (route.query.type === 'playlist_share') {
...
const handleScroll = (event?: Event) => {
  ...
  const isCloudUserPlaylist = route.query.type === 'cloud_user'
    // disabled: cloud — cloud_user fetching disabled
    // if (isCloudUserPlaylist) {
    //   fetchCloudUserPlaylist(false)
    // } else
    if (isCloudUserPlaylist) {
      // no-op: cloud fetching disabled
...
const moreActions = computed(() => [
  ...
  // disabled: cloud/sharing - uploadToCloud removed
  // disabled: cloud — saveToLocal menu removed
  // {
  //   label: '保存到本地',
  //   key: 'saveToLocal',
  ...
  // },
  // disabled: cloud/sharing - syncToCloud, syncFromCloud removed
])
function handleMoreAction(key: string) {
  ...
  // disabled: cloud/sharing - uploadToCloud, syncToCloud, syncFromCloud removed
