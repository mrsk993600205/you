...
const STORAGE_KEY = 'ikunmusic_share_consent'
const LEGAL_DOC_URL =
...
const techPluginUpload =
  '你当前使用的音源插件代码会被上传到Ikun music服务器，按 md5 全局去重保存。服务器对代码做静态危险代码扫描（拦截 fs / child_process / eval 等越权调用）并在隔离 Worker 中运行，但不审核插件本身的音乐版权合法性。'
const techMetadata =
  '歌曲信息、歌词、热评保存于服务器，按你设置的有效期到期自动删除；过期后链接立即失效。'
const techAudio =
  '浏览器请求音频时，服务器临时运行你上传的插件解析出第三方平台直链，以 HTTP 302 重定向让浏览器直接连第三方 CDN——音频流不经过、不缓存、不存储于Ikun music服务器。能否成功播放、播放的是哪一段音频，完全取决于你提供的插件与第三方平台。'
const techPlaylistDefault =
  ' 协议的导入链接，访问者在Ikun music App 中打开导入；不调用任何插件、不解析音频。'
