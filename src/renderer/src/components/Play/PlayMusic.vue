...
watch(
  ...
  (newUrl) => {
    if (dlnaStore.currentDevice && newUrl) {
      ...
      dlnaStore.play(newUrl, songInfo.value.name || 'IkunMusic').then(() => {
...
// disabled: LT - listenTogetherEntryVisible, listenTogetherEntryMode, listenTogetherStore, showListenTogether, LT watches, lockedByLT, guardLT
...
// watch(showFullPlay, (v) => { listenTogetherStore.setFullPlayVisible(v) }, { immediate: true })
// watch(() => listenTogetherStore.closeFullPlayRequested, ...)
// const lockedByLT = computed(...)
// function guardLT(): boolean { ... }

/* 三个核心播放按钮 —— 无 LT 守卫，直接调用 */
...
const moreMenuOptions = computed(() => {
  const opts: any[] = [
    // disabled: cloud/sharing - share menu item removed
  ]
...
const handleMoreMenuSelect = (key: string) => {
  // disabled: cloud/sharing - share case removed
  /* disabled: LT — 一起听菜单分发
  if (key === 'lt:create') {
...
// 无感过渡预告区间在进度条上的百分比位置（无感过渡已移除，标记隐藏）
const crossfadeMarkVisible = computed(() => false)
const crossfadeMarkLeft = computed(() => 0)
const crossfadeMarkWidth = computed(() => 0)
// 过渡激活时也标注实际正在发生淡化的区段
const crossfadeActiveMarkVisible = computed(() => false)
const crossfadeActiveMarkLeft = computed(() => 0)
const crossfadeActiveMarkWidth = computed(() => 0)

// 过渡完成后，新歌开头的淡入区段标记（从 0 到 fadeInMarkEnd 秒）
const crossfadeFadeInMarkVisible = computed(() => false)
const crossfadeFadeInMarkWidth = computed(() => 0)
