 ...
 *
 * 解决问题：本地装了生产版 IkunMusic 时，ikunmusic:// deeplink 会被生产版抢走，
 * 导致 dev:second 实例（用于一起听等多人功能调试）收不到 OAuth 回调。
 *
 * 工作流：
 *   1. 在第二实例正常点"登录"按钮，浏览器跳到 Logto
 *   2. 登录账号 B 完成后，浏览器尝试跳 ikunmusic://callback?code=xxx&state=yyy
 *   3. 由于协议被生产版接管，浏览器要么打开生产版要么报错
 *      —— 关键：这个 URL 在浏览器**地址栏 / 重定向历史里能看到**
 *   4. 复制完整的 ikunmusic://callback?... URL
...
/** 输入有效性：必须以 ikunmusic://callback 开头且包含 code 参数 */
const isValid = computed(() => {
  const u = callbackUrl.value.trim()
  if (!u.startsWith('ikunmusic://')) return false
  // logto callback 必带 code 或 error 参数
  return /[?&](code|error)=/.test(u)
})

/* 自动从剪贴板检测 ikunmusic:// URL —— 用户复制后打开浮窗就自动填上 */
async function detectFromClipboard(): Promise<void> {
  try {
    const text = await navigator.clipboard?.readText()
    if (!text) return
    const m = text.match(/ikunmusic:\/\/[^\s]+/)
...
<template>
  <div v-if="isDev" class="dev-auth-injector" :class="{ expanded }">
    ...
    <div v-else class="panel">
      ...
      <p class="hint">
        粘贴浏览器中拦截到的
        <code>ikunmusic://callback?code=...</code>
      ...
      <t-textarea
        ...
        placeholder="ikunmusic://callback?code=...&state=..."
      />
