...
// import { useAuthStore } from '@renderer/store' // disabled: login
// import { settingsSyncService } from '@renderer/services/SettingsSyncService' // disabled
...
// import { tryShowListenTogetherInvite } from '@renderer/services/listenTogetherInvite' // disabled
...
// const authStore = useAuthStore() // disabled: login
...
const sponsorPromptHiddenKey = 'ikunmusic_sponsor_prompt_hidden_v1'
*/
const usageTotalMs = ref(0)
const usageTotalMsKey = 'ikunmusic_usage_total_ms_v1'
...
const flushUsage = () => {
  ...
  if (delta > 0) {
    ...
    persistUsage()
    // maybeShowSponsorPrompt() // 赞助弹窗已移除
  }
}
...
const parseSonglistNameFromRaw = (rawName: string): string => {
  ...
  const mSonglist = rawName.match(/^ikunmusic-songlist-(.+?)-\d{4}-\d{2}-\d{2}$/i)
  if (mSonglist) parsedName = mSonglist[1]
  else {
    const mSimple = rawName.match(/^ikunmusic-(.+)$/i)
...
onMounted(() => {
  ...
  userInfo.init()
  // settingsSyncService.init() // disabled
...
// disabled: LT deeplink + clipboard check
//   window.electron?.ipcRenderer
          ...
          >
            <div style="max-width: 420px">
              <p style="margin: 0 0 8px 0">
                hi！ 大大Ikun music已经陪伴您
                {{
                  sponsorUsageText
                }}。如果您喜欢Ikun music可以给我们一点小小的支持吗，我相信我们会越做越好哒。
              </p>
