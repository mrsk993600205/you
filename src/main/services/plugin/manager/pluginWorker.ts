...
// ==================== ikunmusic API ====================
function buildIkunmusicApi(): any {
...
function createSandbox(): vm.Context {
  ...
  const ikunmusic = buildIkunmusicApi()
...
async function invokeMethod(method: string, args: any[]): Promise<any> {
  ...
  // Bind ikunmusic API as `this.ikunmusic` (matches the previous host behavior).
  const ikunmusic = (sandboxContext as any).ikunmusic
  return await fn.call({ ikunmusic }, ...args)
}
