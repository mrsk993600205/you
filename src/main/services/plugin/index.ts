...
const pluginService = {
  ...
  async selectAndAddPlugin(type: 'lx' | 'cr') {
    try {
      ...
      const result = await dialog.showOpenDialog({
        title: `请选择你的 ${type == 'lx' ? '洛雪' : 'Ikun music'} js插件`,
        filters: [
      ...
      // 插件格式校验
      if (type === 'cr') {
        // Ikun music格式校验：检查是否包含ikunmusic关键字
        if (!pluginCode.toLowerCase().includes('ikunmusic')) {
          throw new Error('Ikun music插件格式校验失败：代码有可能不是Ikun music格式插件')
        }
      } else if (type === 'lx') {
        // 洛雪格式校验：检查是否包含lx关键字
  ...
  async addPlugin(pluginCode: string, pluginName: string, targetPluginId?: string) {
    try {
      ...
      // 重新加载插件以确保正确初始化
      const ikunmusicPluginManager = new CeruMusicPluginHost()
      ikunmusicPluginManager.pluginId = pluginId
      ikunmusicPluginManager.onThrottle = _throttleHandler
      ikunmusicPluginManager.onDisabled = _disabledHandler
      await ikunmusicPluginManager.loadPlugin(filePath, new Logger(pluginId))

      // 将插件添加到已加载插件列表
      loadedPlugins[pluginId] = ikunmusicPluginManager

      return {
        pluginId,
        pluginName: safePluginName,
        pluginInfo,
        supportedSources: ikunmusicPluginManager.getSupportedSources()
      }
    } catch (error: any) {
      ...
      const results = await Promise.all(
        files.map(async (file) => {
          try {
            ...
            ikunmusicPluginManager.onThrottle = _throttleHandler
            ikunmusicPluginManager.onDisabled = _disabledHandler
            await ikunmusicPluginManager.loadPlugin(fullPath, new Logger(pluginId))

            // 获取插件信息
            const pluginInfo = ikunmusicPluginManager.getPluginInfo()
  ...
  async getPluginsList() {
    ...
    return Object.entries(loadedPlugins).map(([pluginId, manager]) => {
      const ikunmusicPluginManager = manager as CeruMusicPluginHost
      return {
        pluginId,
        pluginName: pluginId.split('-')[1] || pluginId,
        pluginInfo: ikunmusicPluginManager.getPluginInfo(),
        supportedSources: ikunmusicPluginManager.getSupportedSources(),
        disabled: ikunmusicPluginManager.isDisabled()
      }
    })
  },
  ...
  async downloadAndAddPlugin(url: string, type: 'lx' | 'cr', targetPluginId?: string) {
    try {
      ...
      if (type === 'cr') {
        // Ikun music格式校验：检查是否包含ikunmusic关键字
        if (!pluginCode.toLowerCase().includes('ikunmusic')) {
          throw new Error('Ikun music插件格式校验失败：代码中未找到ikunmusic关键字')
        }
      } else if (type === 'lx') {
        // 洛雪格式校验：检查是否包含lx关键字
        if (!pluginCode.toLowerCase().includes('lx')) {
