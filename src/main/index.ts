...
import {
  ...
  // consumePendingLtCodes // disabled
} from './router/routes'
...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient('ikunmusic', process.execPath, [path.resolve(process.argv[1])])
  }
} else {
  app.setAsDefaultProtocolClient('ikunmusic')
}
...
if (!gotTheLock) {
  ...
  app.on('second-instance', (_event, argv) => {
    ...
    const protocolUrl = argv?.find((arg) => arg.startsWith('ikunmusic://'))
...
function setupTray(): void {
  ...
  if (g.__ikunmusic_tray__) {
    tray = g.__ikunmusic_tray__
  } else {
  ...
  // 防重复注册 IPC 监听（仅注册一次）
  if (!g.__ikunmusic_tray_ipc_bound__) {
    ...
    g.__ikunmusic_tray_ipc_bound__ = true
  }

  if (!g.__ikunmusic_tray_before_quit_bound__) {
    app.once('before-quit', () => {
      ...
      if (process.platform !== 'darwin') {
        ...
        tray = null
        g.__ikunmusic_tray__ = null
      }
    })
    g.__ikunmusic_tray_before_quit_bound__ = true
  }
}
...
function createWindow(): void {
  ...
  // disabled: login - Logto auth browser redirect
...
// import { registerAutoUpdateEvents, initAutoUpdateForWindow } from './events/autoUpdate' // disabled
// import { cleanupDownloadedInstallers } from './autoUpdate' // disabled

// 注册自动更新事件 - 尽早注册以避免时序问题 (disabled)
// registerAutoUpdateEvents() // disabled
...
app.whenReady().then(async () => {
  // 清理上次安装残留的安装包（仅限临时目录）
  try {
    // await cleanupDownloadedInstallers() // disabled
  } catch {}
  // Set app user model id for windows - 确保与 electron-builder.yml 中的 appId 一致
  // dev 模式用单独 AUMID，避免 electron.exe 被注册为 com.ikunmusic.app 污染
  // Windows Toast / SMTC / 任务栏分组的缓存。详见对应排障记录。
  const AUMID = is.dev ? 'com.ikunmusic.app.dev' : 'com.ikunmusic.app'
  ...
  */
  ...
  // 初始化自动更新器 (disabled) 桌面歌词
  if (mainWindow) {
    // initAutoUpdateForWindow(mainWindow) // disabled
    lyricWindow.create()
    initLyricIpc(mainWindow)
    const startArg = process.argv?.find((a) => /\.(cmpl|cpl)$/i.test(a))
    if (startArg) queueOpenPlaylist(startArg)
    // 冷启动 deep-link（Windows / Linux）：进程参数中包含 ikunmusic:// 协议链接
    const startProtocolUrl = process.argv?.find(
      (a) => typeof a === 'string' && a.startsWith('ikunmusic://')
    )
    if (startProtocolUrl) {
