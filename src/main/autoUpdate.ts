...
const UPDATE_SERVER = 'https://update.ikunmusic.top'
  ...
  try {
    const res = await fetchWithDohFallback(UPDATE_API_URL, {
      ...
      headers: {
        ...
        'User-Agent': 'IkunMusic-AutoUpdater',
        'X-Arch': CLIENT_ARCH
      },
      timeoutMs: 10000
    })
...
async function detectBlockmapPresence(): Promise<boolean> {
  ...
  try {
    const res = await fetchWithDohFallback(`${UPDATE_SERVER}/${ymlName}`, {
      headers: { 'User-Agent': 'IkunMusic-AutoUpdater' },
      timeoutMs: 5000
    })
...
async function downloadWithDifferential() {
  ...
  try {
    ...
    // update.ikunmusic.top 走实时代理时,单范围差分更稳,避免 multipart/byteranges 兼容性问题
