...
- 加密采用 AES（CryptoJS 的 OpenSSL 兼容密码模式），密钥为 `IkunMusic-PlaylistSecretKey`。
...
2. 使用 CryptoJS 执行 AES 加密（OpenSSL 兼容密码模式，含随机 `Salt`）：
   - 密钥：`IkunMusic-PlaylistSecretKey`
...
- 默认导出文件名：`ikunmusic-playlist-YYYY-MM-DD.cmpl`
