...
> 本文档面向**第三方客户端 / 二次开发者**。如果你只是用Ikun music客户端听歌，去看 [一起听使用指南](./used/listen-together)。
...
- 所有客户端事件名都列在 [src/listen-together/constants.ts](https://github.com/timeshiftsauce/IkunMusic/blob/main/../ceru-backend/src/listen-together/constants.ts) 的 `ClientEvents` 常量;服务端事件在 `ServerEvents`
...
- [Scheme URL](./used/scheme-url) — `ikunmusic://lt/<code>` 落地协议
- [Ikun music 后端 API 总览](./api)
