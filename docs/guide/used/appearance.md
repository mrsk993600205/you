...
- **文档目的：** 指导用户在 Ikun music 中自定义应用外观，包括标题栏风格、主题色、全局背景、歌词字体及桌面歌词样式。
...
Ikun music 支持两种标题栏控制按钮风格，可根据个人喜好或操作习惯选择。
...
A：部分情况下需要重启应用后生效，请尝试关闭并重新打开 Ikun music。

**Q2：关闭按钮的「最小化到托盘」开启后，在哪里找到应用图标重新打开？**

A：应用最小化到托盘后，在系统任务栏右侧（Windows）或菜单栏（macOS）的通知区域找到 Ikun music 图标，双击或右键点击图标选择「显示主窗口」即可恢复。
