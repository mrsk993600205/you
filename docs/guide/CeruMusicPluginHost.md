...
# IkunMusicPluginHost 使用文档

## 概述

IkunMusicPluginHost 是一个用于加载和执行 IkunMusic 插件的 JavaScript 引擎。它提供了一个安全的沙箱环境来运行插件代码，并提供标准化的 API 接口。
...
```javascript
const IkunMusicPluginHost = require('./IkunMusicPluginHost')
...
const host = new IkunMusicPluginHost(pluginCode)

// 方式2: 从文件加载
const host2 = new IkunMusicPluginHost()
...
`new IkunMusicPluginHost(pluginCode)`
...
```javascript
// IkunMusic API
ikunmusic.env // 运行环境标识
ikunmusic.version // 版本号
ikunmusic.request // HTTP 请求函数
ikunmusic.utils // 工具函数集合
...
插件可以使用 `ikunmusic.request` 进行网络请求：

```javascript
// 支持 callback 模式
ikunmusic.request(url, options, (error, response) => {
  if (error) {
...
// 也支持 Promise 模式
const response = await ikunmusic.request(url, options)
```
...
1. **插件加载错误**
   ...
   ```javascript
   try {
     const host = new IkunMusicPluginHost(invalidCode)
   } catch (error) {
