...
1. **IkunMusic 原生插件**：基于 IkunMusic API 的插件格式
...
首先 `Ikun music` 插件是面向 方法的 这意味着你直接导出方法即可为播放器提供音源
...
/**
 * @name 示例音乐源
 * @description IkunMusic 原生插件示例
 * @version 1.0.0
 * @author IkunMusic Team
 */
...
async function musicUrl(source, musicInfo, quality) {
  try {
    // 使用 ikunmusic API 发送 HTTP 请求
    const result = await ikunmusic.request('https://api.example.com/music', {
      method: 'POST',
      headers: {
...
async function getPic(source, musicInfo) {
  try {
    const result = await ikunmusic.request(`https://api.example.com/pic/${musicInfo.id}`);
...
async function getLyric(source, musicInfo) {
  try {
    const result = await ikunmusic.request(`https://api.example.com/lyric/${musicInfo.id}`);
...
/**
 * @name 示例音乐源
 * @description IkunMusic 原生插件最小模板
 */
...
async function musicUrl(source, musicInfo, quality) {
  ...
  const resp = await ikunmusic.request(endpoint, {
    method: 'POST',
...
async function getPic(source, musicInfo) {
  const id = musicInfo.id || musicInfo.songmid || musicInfo.hash
  const resp = await ikunmusic.request(`https://api.example.com/${source}/pic/${id}`, {
    timeout: 10000
  })
...
async function getLyric(source, musicInfo) {
  const id = musicInfo.id || musicInfo.songmid || musicInfo.hash
  const resp = await ikunmusic.request(`https://api.example.com/${source}/lyric/${id}`, {
    timeout: 10000
  })
...
function signPayload(payload, secret) {
  ...
  return ikunmusic.utils.crypto.md5(base)
}
```
...
async function cachedMusicUrl(source, musicInfo, quality, ttl = 300000) {
  ...
  const resp = await ikunmusic.request('https://api.example.com/url', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
- 在 设置 → 插件管理 中导入插件文件
- 在 插件管理 → 日志 查看调试输出
- 使用 `ikunmusic.NoticeCenter('update', ...)` 推送更新提示
...
- 初始化：注入 `ikunmusic` API、`Buffer`、`JSON`、计时器函数等必要对象
- 调用：通过宿主统一调用 `musicUrl/getPic/getLyric` 并进行异常捕获与日志记录
- 卸载：删除插件文件并清理内存中的实例

#### 沙箱环境与安全

- 全局可用对象：`ikunmusic`、`console`、`setTimeout/clearTimeout`、`setInterval/clearInterval`、`Buffer`、`JSON`
- 禁止直接 `require` 外部模块（由宿主注入空实现）
- `ikunmusic.utils.crypto` 支持：
...
- 安装来源：
  - 本地选择文件：会进行格式关键字校验（Ikun music需包含 `ikunmusic`，洛雪需包含 `lx`）
  - URL 下载：30 秒下载超时，空文件拒绝
  - LX 事件驱动插件将自动转换为Ikun music方法格式
...
```javascript
// Buffer 操作
ikunmusic.utils.buffer.from(data, encoding)
ikunmusic.utils.buffer.bufToString(buffer, encoding)

// 加密工具
ikunmusic.utils.crypto.md5(str)
ikunmusic.utils.crypto.randomBytes(size)
ikunmusic.utils.crypto.aesEncrypt(data, mode, key, iv)
ikunmusic.utils.crypto.rsaEncrypt(data, key)
```

#### ikunmusic.NoticeCenter(type, data)

发送通知到用户界面：

```javascript
ikunmusic.NoticeCenter('info', {
  title: '通知标题',
  content: '通知内容',
...
/*!
 * @name Ikun music源
 * @description Ikun music插件 支持酷我，网易，咪咕平台最高flac
 * @version v4.0.0
 * @author 时迁酱
 * @homepage https://example.com
 */
...
const apiUrl = ''
// 获取工具函数
const { request, NoticeCenter } = ikunmusic
    ...
    const result = await request(
      `${apiUrl}/music/url?source=${source}&songId=${songId}&quality=${quality}`,
      {
        ...
        headers: {
          ...
          'User-Agent': `IkunMusic-Plugin/${pluginInfo.version}`
        },
        timeout: 15000
      }
    )
...
IkunMusic 可以兼容 LX Music 的插件格式，支持事件驱动的开发模式。
...
async function musicUrl(source, musicInfo, quality) {
  try {
    ...
    // API 调用
    const result = await ikunmusic.request(url, options)
...
```javascript
const result = await ikunmusic.request(url, {
  timeout: 10000 // 10秒超时
})
```
...
async function limitedRequest(url, options) {
  ...
  try {
    return await ikunmusic.request(url, options)
  } finally {
    semaphore.release()
  }
}
...
A: IkunMusic 的请求方法不受浏览器跨域限制，可以直接请求任何域名的 API。

### Q: 插件如何更新？

A: 使用 `ikunmusic.NoticeCenter` 事件通知用户更新：

```javascript
ikunmusic.NoticeCenter('update', {
  title: '新版本更新',
  content: 'xxxx',
  version: 'v1.0.3',
...
- GitHub Issues: [IkunMusic Issues](https://github.com/timeshiftsauce/IkunMusic/issues)
- Blog (最好登录，否则需要审核): [IkunMusic Blog](https://shiqianjiang.cn/blog/4966904626407280640)
